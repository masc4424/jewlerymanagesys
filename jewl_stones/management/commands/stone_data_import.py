...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--user', type=str, default=None,
                          help='Username to set as created_by and updated_by (defaults to first superuser)')
    ...
    def handle(self, *args, **options):
        ...
        # Get or determine the user for created_by/updated_by fields
        user = self.get_import_user(username)
        if not user:
            self.stdout.write(self.style.ERROR('No valid user found for import. Please specify --user or ensure at least one superuser exists.'))
            return
        
        self.stdout.write(self.style.SUCCESS(f'Using user "{user.username}" for created_by/updated_by fields'))
        ...
        for _, row in df.iterrows():
            ...
            if stone_name and stone_name.lower() != 'nan':
                ...
                if stone_name_key not in processed_stones:
                    # Create a new stone with user tracking
                    current_stone = Stone.objects.create(
                        name=stone_name,  # Keep original case when saving
                        is_active="ACTIVE",  # Set to ACTIVE as shown in your database
                        created_by=user,
            ...
            if size or weight > Decimal('0') or rate > Decimal('0'):
                if current_stone and current_stone_type:
                    ...
                    # Create stone type detail with proper precision for decimal values and user tracking
                    StoneTypeDetail.objects.create(
                        length=length,
                        breadth=breadth,
                        weight=weight,  # This will maintain the exact decimal value
    ...
    def get_import_user(self, username):
        """Get the user to use for created_by/updated_by fields"""
        if username:
            try:
                return User.objects.get(username=username)
            except User.DoesNotExist:
                self.stdout.write(self.style.WARNING(f'User "{username}" not found. Trying to use first superuser.'))
        
        # If no username provided or user not found, try to get the first superuser
